import { listReferences, readReferenceChapter } from './reference.mjs'

export const STUDY_VERSION = '0.4.0'
...
export function buildStudyPackage(textType = 'mixed', mode = 'authoring') {
  ...
  const chapters = order.map(({ file }) => ({ name: file, text: readReferenceChapter(file) }))
  ...
  return {
    ...
    before_writing: [
      '明确这次要写什么，要改变读者的什么；不产出内部推理，允许保存有依据的内容记忆和简洁任务交接。',
    ...
    final_note: '按当前任务使用理论，遇到问题可以回查。阅读不等于理解，不以工具调用成功证明文笔。重要进展保存简洁交接。',
  }
}
