...
function scanForbidden(text) {
  ...
  scan('em-dash', /——|—/g, '破折号——（§18 清单第1条：应计数为 0，对话打断用动作/短句承接）')
  scan('halfwidth-quote', /"/g, '半角引号"（§18 清单第2条：应为 0，对话统一用全角""）')
  scan('self-naming-emotion', /我是[^，。！？]{1,8}的/g, '"我是X的"命名情绪句式（§18 清单第3条：应为 0，情绪改用动作/停顿/物件承载）')
  scan('psych-proxy', /我这才明白|我先前以为|我才明白|意识到|才想明白/g, '心理代理套路（§18 清单第4条：我这才明白/我先前以为/意识到，认知改用物件或行为自证）')
...
export function profile(text) {
  ...
  return {
    meta: {
      ...
      note: '分布画像 + 内容锚点；供人味化诊断阶段参考，非"去修"指令。',
    },
