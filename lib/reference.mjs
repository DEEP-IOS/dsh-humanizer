...
// humanize_reference 工具调用，从插件自身目录读取并返回章节全文。
...
/**
 * 按章节号（"05"）或文件名关键词（"十维"）读取章节全文。
 * @param {string} query - 章节号（00—18）或文件名片段。
 * @returns {{name:string,text:string}|{available:string[]}|{error:string,available:string[]}}
 */
export function readReference(query) {
  const files = listReferences()
  const q = String(query ?? '').trim()
  if (!q) {
    return { available: files, note: '未指定章节；传入 00—18（如 "05"、"00-工作流"）。' }
  }
