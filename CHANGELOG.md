...
- 补齐安装/升级/回退、配置、工具契约、案例、排错、开发、测试与发布指南，增加统一导航。
- 重写现行理论与设计说明，保留 v0.3 快照；统一参考库按需阅读、来源记忆和公开交接协议，退役检测信号式审核。
- 明确召回、同 key 冲突、预算、并发交接和遗忘范围，修正未经验证的效果承诺与研究出处。
- 固定参考 Markdown 为 LF，避免 Windows 换行导致小节查询失败。
- 发布包包含 CONTRIBUTING 和 SECURITY；next 使用本版，latest 保持 0.3.0-rc.2。
- JavaScript 工具实现与 schema 1 沿用 alpha.1，无数据迁移；模型读取的参考 Markdown 有更新，未声称通过成稿盲评。

## [0.4.0-alpha.1] - 2026-09-06

- 新增本机 SQLite 证据关系图：工作区/作品隔离，定稿/草稿/设想与明示/推断标注、同 key 冲突候选、历史、版本校验和遗忘。
- 新增 humanize_prepare / humanize_memory / humanize_checkpoint；通过 dsh 动态 user-role context 恢复已绑定作品资料与任务。
- 按需读取相关章节全文，保留完整 21 章；同步修订 v0.3“读全即理解”和“禁止一切工件”的矛盾协议。
- 设置页新增可编辑、可复制的继续/润色/查看/修正/遗忘请求模板。
- 增加持久化、隔离、证据、并发、冲突、关系扩展、预算、遗忘测试和官方宿主集成；最低 Node 固定验证 22.19.0。
- 这是机制升级的 alpha；不声称通过成稿盲评或全球质量排名。npm 发布与 GitHub 源码更新分开。


## [0.3.0-rc.2] - 2026-09-06

- 修复 DSH STORE #522：声明 Node.js 范围和三个经实际验证的 DSH 完整版本，`0.1.3-alpha.1` 保持待验证。
- 修正浏览器依赖的包名与服务名混用，设置页标签使用动态函数，采用宿主主题颜色。
- 理论文档共享只读缓存，调用结果相互隔离；修正参考读取器仍显示 00—18 的旧提示。
- 工具支持宿主并行调度，补充 PTC 与长结果转存的阅读提示；关闭工具时同步关闭依赖工具的工作流。
- 增加真实 Cordis 工具/卸载/重载、官方 PTC worker、一次性 Web Profile 安装/启动/卸载测试，以及三版本 Linux/Windows CI。
